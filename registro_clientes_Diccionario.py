...
registro_clientes_Diccionario.py 
...
Modificaciones:
------
• coloque la acción de ordenar la lista pincipal antes de guardar en el archivo de texto. Así se guarda ordenada la, información.
• al momento de usar el bucle "for" para guardar, añadí la ubicación de cada valor de la sub-lista, lo que cambia la presentación en el archivo de texto.
• dada la modificación al guardar, en el archivo, cada línea se lee: "name, lastname, email". Los valores aparecen separados por una coma (",") y no hay corchetes.
• la nueva presentación genera una facilidad para leer el contenido. Incluso podría imprimirse o compartirse y ser fácil de leer para alguien que no tenga relación con la programación.
...
while True:
    ...
    #Añadir diccionario a la lista principal 
...
#     se guarda como unn diccionario en cada línea, cada línea representa a un cliente, su apariencia es '[name, lastname, email]'
    ...
    #Guardar como "name, lastname, email"    
    for client in list_clients:
        archivo.write(f"{client['name']}, {client['lastname']}, {client['email']}\n")
    ...
    print("\n\n✅ Cliente registrado")
    print(f"Nombre: {client['name']} Apellido: {client['lastname']} Correo electrónico: {client['email']}")
    ...
    for i, client in enumerate(list_clients, start=1):
        print(f"{i}. Nombre: {client['name']} Apellido: {client['lastname']} Correo electrónico: {client['email']}")
